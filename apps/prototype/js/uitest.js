...
var MCTS = {
  get WebAPIs() {
    delete this.WebAPIs;
    return this.WebAPIs = document.getElementById('WebAPI-tests');
  },
  get APItests() {
 ...
 get iframe() {
  ...
  get WebAPITab() {
    delete this.WebAPITab;
    return this.WebAPITab = document.getElementById('WebAPI');
  },
  get APITab() {
    delete this.APITab;
  ...
  init: function ut_init() {
    ...
    if (name) {
      ...
      // if no test is specified, load WebAPI tests list (select WebAPI tab)
      window.location.hash = 'WebAPI';
    }
  },
  ...
  handleEvent: function ut_handleEvent(ev) {
    switch (ev.type) {
      ...
      case 'unload':
        ...
        break;
      case 'hashchange':
        if (window.location.hash == '#WebAPI')
        {
          this.UItests.classList.remove('invisible');
          AccessibilityHelper.setAriaSelected(this.WebAPITab, this.tabs);
...
window.addEventListener('load', MCTS.init.bind(MCTS));
